...
export type CreditTransactionType = 'INITIAL' | 'ADJUSTMENT' | 'RESERVATION' | 'REFUND' | 'EXTENSION'
...
export const adminUsersApi = {
  ...
  deleteUser: (id: string) =>
    del<{ user: UserDetail }>(`/users/${id}`),

  // Adjust user credits
  adjustCredits: (id: string, data: AdjustCreditsInput) =>
    post<{ user: { id: string; creditBalance: number }; transaction: CreditTransaction }>(
  ...
  getUserReservations: (userId: string, params?: { page?: number; limit?: number }) =>
...
export const invitationsApi = {
  ...
  listPending: () =>
    get<{ invitations: Invitation[] }>('/invitations/pending'),

  // Cancel invitation (admin)
  cancel: (id: string) =>
    del<Record<string, never>>(`/invitations/${id}`),
