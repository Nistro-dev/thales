...
export function SectionTree({
  ...
  const getSubSectionProductCount = (subSection: SubSection & { _count?: { products: number } }) => {
      ...
      {sections.map((section) => {
        ...
        const totalSubSectionProducts =
          section.subSections?.reduce(
            (acc, sub) => acc + getSubSectionProductCount(sub as SubSection & { _count?: { products: number } }),
            0
          ) ?? 0
          ...
          <div key={section.id} className="rounded-lg border bg-card">
            ...
            <div className="flex items-center justify-between p-3">
              <div className="flex items-center gap-2">
                ...
                {section.isSystem && (
                  <span className="text-xs bg-muted px-1.5 py-0.5 rounded">Système</span>
                )}
              ...
              <div className="border-t bg-muted/30">
                {section.subSections?.map((subSection) => {
                  const subProductCount = getSubSectionProductCount(
                    subSection as SubSection & { _count?: { products: number } }
                  )
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1">
                        <Button
                          ...
                          onClick={() => onEditSubSection(subSection, section.id)}
                          title="Modifier"
                        >
