...
export function AvailabilityCalendar({
  ...
  const getDateState = (date: Date, isCurrentMonth: boolean) => {
    ...
    let isDisabled = !isCurrentMonth || isPast || isReserved
    ...
    return { isPast, isReserved, isStart, isEnd, inRange, isDisabled }
  }

  // Can go to previous month?
  const canGoPrevious = useMemo(() => {
    return currentMonth.year > today.getFullYear() ||
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-center gap-2 text-sm">
        {selectingStartDate ? (
          <span className="text-primary font-medium">Sélectionnez la date de sortie</span>
        ) : (
          <span className="text-primary font-medium">Sélectionnez la date de retour</span>
        )}
      ...
      <div className="grid grid-cols-7 gap-1">
        {calendarDays.map(({ date, isCurrentMonth }, index) => {
          const { isPast, isReserved, isStart, isEnd, inRange, isDisabled } = getDateState(date, isCurrentMonth)
          ...
          return (
            <button
              ...
              className={cn(
                ...
                isReserved && isCurrentMonth && 'bg-destructive/20 text-destructive line-through',
                // Selected states
